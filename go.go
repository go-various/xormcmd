...
func typestring(col *core.Column) string {
	st := col.SQLType
	if strings.ToUpper(col.SQLType.Name) == "BIT" {
		return "bool"
	}
	if strings.ToUpper(col.SQLType.Name) == `TINYINT` && col.SQLType.DefaultLength == 1 {
		return "bool"
	}
	if strings.HasPrefix(strings.ToUpper(col.SQLType.Name), "DECIMAL") {
		return "float64"
	}
...
var reg2, _ = regexp.Compile("(\n|\t|\r|^\\s+)")

func tag(table *core.Table, col *core.Column) string {
	isNameId := mapper.Table2Obj(col.Name) == "Id"
	isIdPk := isNameId && (typestring(col) == "int64" || typestring(col) == "int")
	col.Comment = reg2.ReplaceAllString(col.Comment, "")
	col.Comment = reg.ReplaceAllString(col.Comment, " ")
	var res []string
	...
	if genJson > 0 {
		if include(ignoreColumnsJSON, col.Name) {
			...
			//first := string(col.FieldName[0])
			//name := strings.Replace(col.FieldName, first,strings.ToLower(first),1)
			if genJson == 1 {
				tags = append(tags, "json:\""+stripToUpper(col.Name)+"\"")
			} else {
...
func stripToUpper(in string) string {
	out := ""
	found := false
	for i := 0; i < len(in); i++ {
		b := in[i]
